<template>
  <v-container fluid>
    <div class="header">
      <h2><img v-if="setBankName($route.params.product)" :src="getBankImage($route.params.product.kor_co_nm)" alt="Bank Image" class="bank-image">{{ $route.params.product.kor_co_nm }} - {{ $route.params.product.fin_prdt_nm }}</h2>
    ...
    <v-card class="mt-5">
      ...
      <v-card-text>
        <v-row>
          <v-col cols="12" sm="6">
            <p><span class="highlight">기간 : {{ $route.params.product.save_trm }} 개월</span></p>
            <p><span class="highlight">저축 금리: {{ $route.params.product.intr_rate }}%</span></p>
            ...
            <p>적립 유형명: {{ $route.params.product.rsrv_type_nm || '구분 없음' }}</p>
          </v-col>
          <v-col cols="12" sm="6">
            <p><span class="highlight">최고 우대금리: {{ $route.params.product.intr_rate2 }}%</span></p>
            <p>만기 후 이자율: {{ $route.params.product.mtrt_int }}</p>
            <p>최고 한도: {{ $route.params.product.max_limit || '홈페이지 참고' }}원</p>
    ...
    <v-card class="mt-5">
      <v-card-text>
        <v-row>
          <v-col cols="12" sm="6">
            <br>
            <p>{{ $route.params.product.save_trm }} 개월 만기시 세후 수령액 ({{$route.params.product.intr_rate_type_nm }})</p>
            <br>
...
export default {
  ...
  methods: {
    ...
    calculateInterest() {
      ...
      this.bestinterestRate = this.$route.params.product.intr_rate2;
      this.interestRate = this.$route.params.product.intr_rate;
      this.months = this.$route.params.product.save_trm;
